...
───────────
🔍 Ch.3 觀察清單（持有部位）
（先列台股，再列美股。每檔用「・」開頭當第一層 bullet，下一層內容用「　◦ 」當第二層。

【ETF 分析原則】ETF（0050、006208、00895 等）請勿列出成份股清單，只需點出「最關鍵的驅動因子 + 今日結論」，每檔 ETF 最多 2 行（　◦ ），精簡有力。
・ETF 的主要驅動對應：0050 / 006208 → 台積電 + 大盤走勢；00895 → TSLA + EV 族群
...
對台股持股，根據昨夜美股傳導訊號給出今日預判：
- 0050、006208 → 看大盤（S&P 500 + SOX）；ETF 請勿列成份股，只寫關鍵驅動 + 結論，最多 2 行（　◦ ）
- 00895 → 看 TSLA（已有數據）+ EV 族群；同樣最多 2 行，精簡有力
...
　◦ SOX +1.5%、台積電 ADR 溢價正向，預判開盤跟漲
・富邦未來車（00895）
　◦ {last_tw_date} 收：45.66 元
　◦ TSLA 夜漲 +2.3%，EV 族群偏多，預判小幅跳高
・聯發科（2454）
　◦ {last_tw_date} 收：1,450 元
　◦ 夜間訊號：NVDA +2%，AI 晶片族群續強，可望跟漲
・台積電 ADR（TSM 換算）
